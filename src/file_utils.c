...
int parse_color_image(FILE *file, image_file_t *img_file)
{
	...
	for (int line = 0; line < img_file->height; ++line) {
		...
		for (int col = 0; col < img_file->width; ++col) {
			read_ppm_point(file,
						   &(((ppm_point_t **)(img_file->mat))[line][col]),
...
int parse_grayscale_image(FILE *file, image_file_t *img_file)
{
	...
	for (int line = 0; line < img_file->height; ++line) {
		...
		for (int col = 0; col < img_file->width; ++col) {
			read_pgm_point(file,
						   &(((pgm_point_t **)(img_file->mat))[line][col]),
						   img_file->color_max_value > 255 ? 2 : 1);
		}
	}
...
#ifdef MODE_DEBUG
		fprintf(stderr,
				"[debug] Magic constants for PPM image found (0x%x 0x%x)\n",
				magic[0], magic[1]);
#endif
...
#ifdef MODE_DEBUG
		fprintf(stderr,
				"[debug] Magic constants for PGM image found (0x%x 0x%x)\n",
				magic[0], magic[1]);
#endif
...
#ifdef MODE_DEBUG
		fprintf(stderr,
				"[debug] Magic constants for PBM image found (0x%x 0x%x)\n",
				magic[0], magic[1]);
#endif
...
#ifdef MODE_DEBUG
	fprintf(stderr,
			"[debug] Magic constants invalid, found 0x%x 0x%x\n",
			magic[0], magic[1]);
#endif
	...
	for (int i = 0; i < img_file->height; ++i)
		for (int j = 0; j < img_file->width; ++j) {
			...
			if (img_file->color_max_value <= 255) {
				unsigned char value = ((ppm_point_t **)img_file->mat)
									  [i][j].red;
				fwrite(&value, 1, 1, file);
			} else {
				unsigned short value = ((ppm_point_t **)img_file->mat)
									   [i][j].red;
...
void __save_grayscale_image_binary(image_file_t *img_file, char *filename)
{
	...
	for (int i = 0; i < img_file->height; ++i)
		for (int j = 0; j < img_file->width; ++j) {
			if (img_file->color_max_value <= 255) {
				unsigned char value = ((pgm_point_t **)img_file->mat)
									  [i][j].grey;
				fwrite(&value, 1, 1, file);
			} else {
				unsigned short value = ((pgm_point_t **)img_file->mat)
									   [i][j].grey;
				unsigned char byte1 = value / 0xff;
				unsigned char byte2 = value % 0xff;
